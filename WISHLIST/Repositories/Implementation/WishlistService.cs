...
namespace WISHLIST.Repositories.Implementation
{
        ...
        ) : IWishlistService
    {
        ...
        public async Task<List<OwnerWishlistModel>> GetAllOwnWishlistListAsync(string username)
        {
            var user = await _userManager.FindByNameAsync(username);

            return await _dbContext.OwnerWishlists.Where(w => w.OwnerId == user.Id).ToListAsync();
        }
